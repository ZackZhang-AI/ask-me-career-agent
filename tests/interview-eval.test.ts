...
test("报告包含核心覆盖、长度、结构、套话、硬事实与多轮指标", async () => {
  ...
  assert.equal(report.multiTurnResults.length, 3);
  assert.equal(report.multiTurnResults.every((item) => item.turns.length === 3), true);
...
test("上线门禁同时要求角色、质量与硬指标通过", async () => {
  ...
  assert.equal(report.scoring.targetLength, "300-500");
