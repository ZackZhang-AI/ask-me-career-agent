...
export const stableAnswerContent = [
  ...
  answer({
    ...
    standardAnswer: "DeepFlow 是我围绕复杂研究任务设计的多 Agent 工作台。它要解决的问题是：如果把一个开放研究题一次性交给模型，任务很容易在中途跑偏，使用者既不知道它正在做什么，也很难判断最终结论是怎样形成的。\n\n我的产品思路不是简单增加 Agent 数量，而是先把研究过程拆成任务澄清、计划、检索、分析和报告等阶段，再让 Coordinator、Planner、Researcher、Coder 和 Reporter 承担不同职责。对于研究计划和方向调整这类高成本节点，我加入人工确认；对于检索和分析过程，则通过事件记录和引用保留可追踪性。\n\n当前 DeepFlow 已经形成从任务澄清到报告输出的可演示 MVP。这个项目让我更具体地理解：Agent 产品的价值不只来自自主执行，还来自过程是否可见、关键节点是否可控，以及最终成果能否沉淀为可检查、可继续使用的资产。",
  ...
  answer({
    ...
    standardAnswer: "我的技术能力主要服务于产品判断和快速验证，而不是和纯工程师比较底层开发深度。\n\n数据侧，我可以使用 SQL、Python 处理数据、理解指标并检查数据质量；AI 应用侧，我熟悉 RAG、Prompt、工作流、评测集与 RAGAS 等方法，能够把一个模型能力拆成检索、生成、引用和评测环节；工程侧，我能理解 FastAPI、Milvus、前端、异步任务和部署链路，也能借助 AI 编程工具把需求推进成可运行原型。\n\n这种技术宽度对产品工作有两个直接价值。第一，我与算法和工程同学沟通时能够理解实现约束，不会提出完全脱离数据和系统条件的需求；第二，我可以在需求阶段提前定义输入、接口、异常状态和验收方式，并亲自验证关键流程。我的定位不是替代工程师，而是把业务问题翻译成团队能够实现、测试和持续迭代的产品方案。",
  ...
  answer({
    ...
    standardAnswer: "审计经历给我的最大帮助，不是增加一个行业标签，而是让我习惯从真实流程、证据和风险出发看产品。做审计时，我需要关注数据从哪里来、经过哪些处理、过程有没有留下证据、异常如何被发现，以及最终结果能否被另一个人复核。\n\n这些习惯可以直接迁移到企业 AI 产品。设计知识库时，我会关注答案是否基于正确内容、引用能否支持结论；设计 Agent 工作流时，我会区分哪些任务可以自动执行、哪些高成本节点必须人工确认；处理企业数据时，我会先界定授权、使用范围和输出边界。与此同时，审计现场也让我看到，很多流程问题不在于缺少功能，而在于资料分散、交接成本高、异常确认反复。\n\n因此，相比只从模型能力出发，我会更早判断一个方案在真实业务中是否可用、可控、可复核，以及它是否真的降低一线执行成本。这种企业流程理解，是我做 B 端 AI 产品时很重要的差异点。",
  ...
  answer({
    ...
    standardAnswer: "我在 2026 年 1 月至 4 月参与德勤 IT 审计实习，工作主要涉及 IPE 与日志核查。具体来说，我需要按照既定审计程序核对系统输出和相关资料，检查记录之间是否一致，并在口径不清楚或信息不完整时进一步确认。\n\n这段经历让我比较具体地理解了企业系统中的数据来源、流程控制和证据链。一个结果是否可信，往往不只取决于最后一张报表，还取决于数据从哪里产生、经过哪些处理、谁在什么节点确认，以及异常是否被完整记录。它也让我意识到，企业产品不能只追求功能可用，还要考虑结果是否可追溯、错误能否被及时发现，以及敏感数据应该如何被限制使用。\n\n这些认识后来直接影响了我的 AI 产品设计：知识库需要引用支持，Agent 的关键动作需要人审，模型处理的数据也必须有清晰边界。",
  ...
  answer({
    ...
    standardAnswer: "我在 2025 年 1 月至 3 月参与容诚财务审计实习，主要接触底稿、函证和盘点等基础审计工作。现场工作让我看到，一项看起来简单的流程，背后往往包含资料收集、多人协作、异常确认和结果留痕；真正消耗时间的，也常常不是某个复杂判断，而是信息格式不一致、上下文缺失和反复确认。\n\n这段经历对我做产品最直接的影响，是让我更关注一线执行成本。设计企业工具时，我不会只问功能能不能实现，还会考虑资料如何进入系统、用户在哪一步最容易出错、系统应该给出建议还是直接执行、哪些结果必须人工确认，以及操作之后能否留下可复核记录。\n\n它也为我后来把审计资料整理和日志核查抽象成产品原型提供了业务基础：先找到重复而高频的环节，再用规则、OCR 或模型能力降低整理成本，同时保留必要的人审和数据边界。",
  ...
  answer({
    ...
    standardAnswer: "我处理隐私和企业数据的原则是：先确定数据能不能进入系统，再讨论模型能做什么。具体设计时，我会依次确认数据来源与授权、完成任务所需的最小范围、数据在哪里处理、结果会被发送到哪里，以及出现异常后能否追溯和撤回。\n\n在本地优先工具中，我尽量让数据保留在设备内，并在文件移动或自动处理前保留用户确认；在审计相关原型中，我只使用演示数据，不接触真实客户资料；在 Agent 或知识库流程中，则通过人工确认、引用支持和明确的输出边界降低误用风险。如果某项功能必须依赖更敏感的数据，我会先判断能否通过脱敏、范围缩小或本地处理实现，而不是默认把数据交给模型。\n\n对企业 AI 产品来说，隐私不是上线前补充的一页说明，而是需求范围、交互流程、权限和异常处理共同组成的产品约束。",
  ...
  answer({
    ...
    standardAnswer: "我的 CET-4 成绩是 619，CET-6 是 520，并完成了部分 ACCA 科目学习。对我而言，这些经历的价值不在于单纯罗列证书，而在于它们能直接支持 AI 产品工作。\n\n英语能力让我可以直接阅读海外模型文档、开源项目说明、AI 产品案例和工程资料，减少经过二手转述后的信息损耗，也便于持续跟踪模型能力与产品范式的变化。ACCA 和会计相关学习，则让我更容易理解财务流程、业务指标、控制要求和企业沟通语言。两者结合后，我能更快在技术资料与 B 端业务之间建立联系，这也是我进入企业 AI 场景时的一项基础能力。",
