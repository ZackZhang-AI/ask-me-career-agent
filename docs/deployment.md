...
- Vercel AI Gateway：Production 优先使用自动刷新的 OIDC 身份；本地或非 Vercel 环境使用 `AI_GATEWAY_API_KEY`。凭证不进入聊天正文、命令输出、Git、浏览器 Bundle 或应用日志。
...
| 变量 | 必需 | 默认值 / 说明 |
| --- | --- | --- |
| `AI_GATEWAY_API_KEY` | 本地/非 Vercel 必需 | AI Gateway 服务端凭证；Vercel Production 可省略并使用 OIDC |
| `AI_PRIMARY_MODEL` | 否 | `deepseek/deepseek-v4-flash`，负责问题规划与回答生成 |
| `AI_FALLBACK_MODEL` | 否 | `deepseek/deepseek-v4-pro`，负责故障回退与最终强制审校 |
...
2. 打开 Vercel Preview，验证首页、移动端、核心 20 题、开放题、澄清、拒答、引用展开和全部外链。Preview 不应使用 Production 数据库或高预算凭证。
...
7. 使用真实 Gateway 连续验证至少 30 道混合题，确认 Flash/Pro 路径无 401/403、可答开放题不走本地兜底且每条展示答案均完成 Pro 审校。
8. 合并 Pull Request，由 Vercel 生成 Production 部署；不要绕过失败的 CI、Gateway 客户验证或支付配置直接提升 Preview。
...
- 桌面与 390px 移动端无横向溢出；请求建立后 100ms 内出现真实处理阶段，正文只在 Pro 审校通过后逐字展示。
- `/resume`、GitHub、来源、邮箱和电话入口可用；联系方式不进入模型上下文或匿名事件详情。
- 401/403 视为发布失败；429、预算耗尽、上游超时和流中断均显示稳定、可理解的状态，不展示未经审校的回答。
