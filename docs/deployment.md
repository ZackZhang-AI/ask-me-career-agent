...
- DeepSeek 官方 API：Preview 与 Production 通过服务端 `DEEPSEEK_API_KEY` 直连。凭证不进入聊天正文、命令输出、Git、浏览器 Bundle 或应用日志。
...
敏感变量只写入 Vercel Environment Variables，不提交 `.env` 文件。Preview 和 Production 使用独立预算；模型密钥可同时作用于两种环境，但 Preview 不复用 Production 的 Redis 或 Neon 数据。

| 变量 | 必需 | 默认值 / 说明 |
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | 是 | DeepSeek 官方 API 服务端密钥，必须标记为 Sensitive |
| `DEEPSEEK_BASE_URL` | 否 | `https://api.deepseek.com` |
| `AI_PRIMARY_MODEL` | 否 | `deepseek-v4-flash`，负责问题规划与回答生成 |
| `AI_FALLBACK_MODEL` | 否 | `deepseek-v4-pro`，负责瞬时故障回退与最终强制审校 |
...
7. 使用真实 DeepSeek API 连续验证至少 30 道混合题，确认 Flash/Pro 路径无 401/402/403、可答开放题不走本地兜底且每条展示答案均完成 Pro 审校。
