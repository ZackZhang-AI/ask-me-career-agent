...
const unsupportedPersonalPattern = /爱好|兴趣|足球队|球队|生日|星座|MBTI|婚姻|对象|家庭|父母|籍贯|住址|身份证|期望薪资|薪资要求|薪酬|到岗时间|什么时候到岗|入职时间|可实习多久|实习时长|千万|百万(?:营收|收入|付费用户|用户)|亿元?|营收|付费用户|用户规模|正式(?:研发)?团队|带领.{0,8}团队|带过.{0,8}人|独立训练|千亿参数|自动驾驶|支付牌照|融资(?:金额|轮次|经历|结果)/i;
...
const productCasePattern = /设计一款|设计一个|设计.{0,12}(?:产品|功能)|产品设计|如何设计|如何改进|MVP|最小可行|功能优先级|需求优先级|用户需求|核心指标|产品方案|改进.{0,12}(?:产品|功能)|如果让你(?:做|设计|负责|接手).{0,20}(?:产品|功能)/i;
...
export function classifyInterviewQuestion(question: string, intent: AnswerIntent, mode: QuestionMode): LocalInterviewClassification {
  ...
  const asksUnsupportedPersonalFact = unsupportedPersonalPattern.test(question)
    && /(?:你|候选人).{0,16}(?:有|拿到|做过|负责过|带领|带过|独立|多少|是否|有没有|曾经|创造|实现)/.test(question);
  if (asksUnsupportedPersonalFact) {
  ...
  const isReasoningQuestion = mode === "candidate_reasoning"
    || isHypothetical
    || situationalPattern.test(question)
    || ["situational_judgment", "product_design", "business_analysis", "estimation", "work_style"].includes(intent);
  if (unsupportedPersonalPattern.test(question)
    && !isReasoningQuestion
    && !["career_planning", "company_motivation", "career_transition"].includes(intent)) {
    return { questionFamily: "candidate_fact", factRisk: "unsupported_personal", answerStrategy: "boundary_bridge", protectedDecision: true };
  }
            ...
            : isHypothetical ? "situational"
              : workStylePattern.test(question) ? "work_style"
                : mode === "candidate_reasoning" && situationalPattern.test(question) ? "situational"
                  : undefined;
  const reasoningFamilies: InterviewQuestionFamily[] = ["situational", "product_case", "business_case", "estimation", "work_style"];
  const explicitReasoningFamily = Boolean(explicitFamily && reasoningFamilies.includes(explicitFamily));
  const resolvedIntentFamily = (intent === "challenge" && (isHypothetical || workStylePattern.test(question)))
    || (intent === "work_style" && (isHypothetical || behavioralPattern.test(question)))
    || (isHypothetical && explicitReasoningFamily)
    ? undefined
    : intentFamily;
  const questionFamily = resolvedIntentFamily
    ?? explicitFamily
    ?? (interviewScopePattern.test(question) ? "candidate_fact" : "unrelated");
  const isReasoningFamily = reasoningFamilies.includes(questionFamily);
  const factRisk: FactRisk = isReasoningFamily ? "low" : mode === "candidate_fact" || questionFamily === "behavioral" ? "supported_personal" : "low";
...
export function shouldPlanWithModel(input: { hasContract: boolean; hasStableAnswer: boolean; classification: LocalInterviewClassification }) {
  ...
  return input.classification.questionFamily !== "agent_meta";
}
