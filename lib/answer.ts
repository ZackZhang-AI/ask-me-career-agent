...
export function buildAnswerPlan(
  ...
  const partialPlan: Omit<AnswerPlan, "fallbackAnswer"> = {
    reviewedAnswerId: reviewedAnswerId === stableAnswer?.id ? reviewedAnswerId : undefined,
  ...
  const reviewedStableAnswer = reviewedAnswerId && reviewedAnswerId === stableAnswer?.id ? stableAnswer : undefined;
  const baseAnswer = reviewedStableAnswer
    ? reviewedStableAnswer.standardAnswer
    : contract?.fallbackAnswer
...
严格遵守 <answer_task> 中的本题任务、结构、长度、新信息目标和避免重复项。第一句控制在 45 个中文字符内，直接回答面试官的问题并能独立成立；不要先铺背景或说客套话。不同问题使用不同表达结构：自我介绍自然叙事；项目回答讲问题、判断、方案和价值；贡献回答讲本人决定、取舍和验收；行为问题使用 STAR；简单事实直接回答。对话进入深层追问后，要先直接回应本轮问题，再调用最相关的具体实践解释判断，不得把材料字段逐段拼接成答案，也不要复述上一轮的项目介绍。不要为了格式强制写三段，也不要每次重复候选人的三项优势。
